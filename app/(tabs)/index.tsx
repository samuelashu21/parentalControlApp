...
function AdminDashboard() {
  ...
  const parentCount = users.filter((managedUser) => managedUser.role === "parent")
    .length;
  const childCount = users.filter((managedUser) => managedUser.role === "child")
    .length;
  ...
  return (
    <ScrollView contentContainerStyle={styles.scroll}>
      <ThemedView style={styles.container}>
        ...
        <ThemedText style={{ marginBottom: 12 }}>Welcome, {user?.name}</ThemedText>
          ...
          <>
            ...
            {users.length === 0 ? (
              ...
              users.map((managedUser) => (
                ...
                >
                  <ThemedText type="defaultSemiBold">{managedUser.name}</ThemedText>
                  <ThemedText style={{ color: colors.icon }}>
                    {managedUser.email}
                  </ThemedText>
