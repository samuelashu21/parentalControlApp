  ...
  const handleLinkChild = async () => {
    ...
    try {
      const res = await authFetch(
        "/auth/link-child",
        { method: "POST", body: JSON.stringify({ childEmail: linkEmail.trim().toLowerCase() }) },
        token
      );
      ...
      setChildren((prev) => {
        ...
        return exists ? prev.map((c) => (c._id === child._id ? child : c)) : [child, ...prev];
      });
      setLinkEmail("");
    ...
    <ThemedView style={styles.flex}>
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        {children.length === 0 ? (
          ...
          children.map((child) => (
            <View key={child._id} style={[styles.childCard, { borderColor: colors.icon }]}>
              <View style={styles.childHeader}>
                ...
                <View
                  style={[
                    ...
                    { backgroundColor: child.isTrackingPaused ? "#e74c3c" : "#2ecc71" },
                  ]}
                >
              ...
              <ThemedText style={{ color: colors.icon }}>{child.email}</ThemedText>
              ...
              <View style={styles.infoRow}>
                <ThemedText>
                  Consent:{" "}
                  <ThemedText style={{ color: child.trackingConsent ? "#2ecc71" : "#f39c12" }}>
              ...
              <TouchableOpacity
                style={[
                  styles.toggleBtn,
                  {
                    backgroundColor: child.isTrackingPaused ? "#2ecc71" : "#e74c3c",
              ...
              >
                {toggling === child._id ? (
                  ...
                  <ThemedText style={styles.toggleBtnText}>
                    {child.isTrackingPaused ? "▶ Resume Tracking" : "⏸ Pause Tracking"}
                  </ThemedText>
                )}
              </TouchableOpacity>
            </View>
...
const styles = StyleSheet.create({
  ...
  childCard: { borderWidth: 1, borderRadius: 12, padding: 16, marginBottom: 14, gap: 6 },
  ...
  infoRow: { marginTop: 2 },
  toggleBtn: { paddingVertical: 10, borderRadius: 8, alignItems: "center", marginTop: 10 },
