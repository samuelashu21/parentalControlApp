...
export default function GeofencesScreen() {
  ...
  const handleCreate = async () => {
    ...
    if (isNaN(lat) || isNaN(lng) || isNaN(radius)) {
      Alert.alert("Validation", "Latitude, longitude, and radius must be numbers.");
      return;
    }
    if (radius < 50) {
    ...
    try {
      const res = await authFetch(
        ...
        {
          method: "POST",
          body: JSON.stringify({ name: formName.trim(), latitude: lat, longitude: lng, radius }),
        },
        token
      );
      const data = await res.json();
  ...
  const handleDelete = async (fence: Geofence) => {
    ...
    Alert.alert("Delete Geofence", `Delete "${fence.name}"?`, [
      { text: "Cancel", style: "cancel" },
      {
        ...
        onPress: async () => {
          try {
            const res = await authFetch(`/geofences/${fence._id}`, { method: "DELETE" }, token);
            if (!res.ok) {
              const data = await res.json();
    ...
    <ThemedView style={styles.flex}>
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        {loading ? (
          ...
          geofences.map((fence) => (
            <View key={fence._id} style={[styles.fenceCard, { borderColor: colors.icon }]}>
      ...
      <Modal visible={modalVisible} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <ThemedView style={styles.modalContainer}>
            ...
            <ThemedText style={styles.label}>Radius (metres, min 50)</ThemedText>
            <TextInput
              style={[styles.input, { color: colors.text, borderColor: colors.icon }]}
              placeholder="200"
              placeholderTextColor={colors.icon}
...
const styles = StyleSheet.create({
  ...
  fenceCard: { borderWidth: 1, borderRadius: 12, padding: 16, marginBottom: 12, gap: 6 },
  fenceHeader: { flexDirection: "row", justifyContent: "space-between", alignItems: "center" },
  fenceName: { flex: 1 },
  ...
  fenceActionBtn: { flex: 1, borderWidth: 1, borderRadius: 8, paddingVertical: 6, alignItems: "center" },
  modalOverlay: {
    flex: 1,
    backgroundColor: "rgba(0,0,0,0.5)",
    justifyContent: "flex-end",
  },
  modalContainer: { padding: 24, borderTopLeftRadius: 20, borderTopRightRadius: 20 },
