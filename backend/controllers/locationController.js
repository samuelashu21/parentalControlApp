...
exports.addLocation = async (req, res) => {
  try {
    if (req.user.role !== "child") {
      return res.status(403).json({ error: "Only children can submit locations" });
    }
...
exports.getLocationHistory = async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(403).json({ error: "Not authorised to view this child's locations" });
    }
...
exports.getLatestLocation = async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(403).json({ error: "Not authorised to view this child's location" });
    }
    if (req.user.role === "child" && String(req.user._id) !== String(childId)) {
      return res.status(403).json({ error: "Not authorised" });
    }

    const location = await Location.findOne({ childId }).sort({ timestamp: -1 });
...
exports.deleteLocationHistory = async (req, res) => {
  try {
    if (req.user.role !== "parent") {
      return res.status(403).json({ error: "Only parents can delete location history" });
    }
    const { childId } = req.params;
    const child = await User.findById(childId);
    if (!child) return res.status(404).json({ error: "Child not found" });
    if (String(child.linkedParent) !== String(req.user._id)) {
      return res.status(403).json({ error: "Not authorised to delete this child's locations" });
    }
    const result = await Location.deleteMany({ childId });
    res.json({ message: "Location history deleted", count: result.deletedCount });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
};
