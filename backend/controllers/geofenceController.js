...
exports.createGeofence = async (req, res) => {
  try {
    if (req.user.role !== "parent") {
      return res.status(403).json({ error: "Only parents can create geofences" });
    }
...
exports.getGeofences = async (req, res) => {
  try {
    ...
    const geofences = await Geofence.find({ parentId: req.user._id }).sort({ createdAt: -1 });
    res.json({ geofences });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
exports.checkGeofence = async (req, res) => {
  try {
    ...
    const geofences = await Geofence.find({ parentId: req.user._id, isActive: true });
